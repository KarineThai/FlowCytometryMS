GetPercentPositive5mark = function(data, id, positivitytable, markercol, posvaluecol, clustercol) {
  #' 
  ...
  result = data %>%
    ...
    summarise(
      ...
      !!sym(paste0(positivitytable[4, markercol])) := mean(get(paste0(positivitytable[4, markercol])) > as.numeric(positivitytable[4, posvaluecol])) * 100,
      !!sym(paste0(positivitytable[5, markercol])) := mean(get(paste0(positivitytable[5, markercol])) > as.numeric(positivitytable[5, posvaluecol])) * 100
    )
